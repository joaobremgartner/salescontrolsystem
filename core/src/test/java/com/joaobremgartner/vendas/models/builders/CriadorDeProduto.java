...
public class CriadorDeProduto {
	...
	public static CriadorDeProduto novoProduto(Long id, String nome, BigDecimal valorVenda, BigDecimal valorCusto,
			int qtd) {
	...
	private static Produto criandoNovoProduto(Long id, String nome, BigDecimal valorVenda, BigDecimal valorCusto,
			int qtd) {
		Produto produto = new Produto();

		populaProdutoComDados(produto, id, nome, valorVenda, valorCusto, qtd);
	...
	public CriadorDeProduto mais(int qtd) {
		...
		for (int i = 0; i < qtd; i++) {
			this.produtos.add(criandoNovoProduto(prdBase.getId() + i, prdBase.getNmProduto(), prdBase.getVlrVenda(),
					prdBase.getVlrCusto(), prdBase.getMaxEstoque() + i));
		}
		return this;
	}
	...
	private static void populaProdutoComDados(Produto produto, Long id, String nome, BigDecimal valorVenda,
			BigDecimal valorCusto, int qtd) {
		...
		// Caso haja alteração no valor da categoria, terá que mudar também na
		// classe de testes
		produto.setCategoria(CategoriaProduto.COSMETICOS);

		Fornecedor fornecedor = CriadorDeFornecedor.novoFornecedorPF(1L, "João Faro", "123.456.789-00", "1234567")
				.criar();
